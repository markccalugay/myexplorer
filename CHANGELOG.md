...
### Changed
- 2026-03-22: Hardened Google Maps configuration recovery after the fail-closed API-key migration by resolving the key from shared runtime config helpers, adding an HTML meta fallback for Vite-provided keys, reusing the same config path for `mapId`, and clarifying in-app guidance that local env changes require a Vite restart or rebuild.
